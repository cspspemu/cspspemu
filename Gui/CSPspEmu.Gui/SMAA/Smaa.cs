...
namespace CSPspEmu.Gui.SMAA
{
	public class Smaa : IDisposable
	{
		...
		private static readonly string SMAA_H = Assembly.GetExecutingAssembly().GetManifestResourceStream("CSPspEmu.Gui.SMAA.SMAA.h").ReadAllContentsAsString();
		private static readonly byte[] SMAA_AREA_RAW = Assembly.GetExecutingAssembly().GetManifestResourceStream("CSPspEmu.Gui.SMAA.smaa_area.raw").ReadAll();
		private static readonly byte[] SMAA_SEARCH_RAW = Assembly.GetExecutingAssembly().GetManifestResourceStream("CSPspEmu.Gui.SMAA.smaa_search.raw").ReadAll();
