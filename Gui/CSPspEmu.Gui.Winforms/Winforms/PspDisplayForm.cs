        ...
        private void UpdateTitle()
        {
            try
            {
                if (LastText != ElfConfig.GameTitle)
                {
                    ...
                    if (ShowMenus)
                    {
                        this.Text = "Soywiz's PspEmu - " + PspGlobalConfiguration.CurrentVersion + " :: r" +
                                    PspGlobalConfiguration.CurrentVersionNumeric + " :: " + Platform.Architecture +
                                    " :: " + LastText;
                    }
                    else
                    {
        ...
        */
        ...
        private void websiteToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            Process.Start(@"http://pspemu.soywiz.com/?rf=csp&version=" + PspGlobalConfiguration.CurrentVersion +
                          "&version2=" + PspGlobalConfiguration.CurrentVersionNumeric);
        }
        ...
        private void dumpRamToolStripMenuItem_Click(object sender, EventArgs e)
        {
            PauseResume(() =>
            {
                ...
                if (SaveFileDialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    Stream.WriteStream(new PspMemoryStream(Memory).SliceWithBounds(PspMemory.MainSegment.Low,
                        PspMemory.MainSegment.High - 1));
                    Stream.Flush();
                    Stream.Close();
                }
            });
        ...
        private void openToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            PauseResume(() =>
            {
                OpenFileDialog.Filter =
                    "Compatible Formats (*.elf, *.pbp, *.iso, *.cso, *.dax, *.prx)|*.elf;*.pbp;*.iso;*.cso;*.dax;*.prx|All Files|*.*";
        ...
        private void OpenFileRealOnNewThreadLock(string FilePath)
        {
            var BackgroundThread = new Thread(() =>
            {
                ...
                PauseResume(() => { OpenFileReal(FilePath); });
            });
        ...
        private void DoInvoke(Action Action)
        {
            if (this.InvokeRequired && GuiThread != Thread.CurrentThread &&
                (!Platform.IsMono || Platform.OS != OS.Windows))
                //if (this.InvokeRequired && GuiThread != Thread.CurrentThread)
            {
                this.Invoke(Action);
            }
        ...
        {
            PauseResume(() => { new AboutForm(this, IGuiExternalInterface).ShowDialog(); });
        }
                ...
                try
                {
                    var Request =
                        HttpWebRequest.Create(
                            new Uri("https://raw.github.com/soywiz/cspspemu/master/version_last.txt"));
                    Request.Proxy = null;
                    ...
                    if (string.CompareOrdinal(ComparableCurrentVersion, ComparableLastVersion) > 0)
                    {
                        if (MessageBox.Show(
                                ...
                                "",
                                "New Version", MessageBoxButtons.OKCancel, MessageBoxIcon.Exclamation) ==
                            DialogResult.OK)
                        {
                    ...
                    else
                    {
                        if (NotifyIfNotFound)
                        {
                            MessageBox.Show("You have the lastest version", "Done", MessageBoxButtons.OK,
        ...
        private void associateWithPBPAndCSOToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Can't associate\n\n" + Result.ErrorString, "Done", MessageBoxButtons.OK,
        ...
        {
            Process.Start(@"https://github.com/soywiz/cspspemu/issues/new?title=" +
                          HttpUtility.UrlEncode("Problem with " + ElfConfig.GameTitle + "\n") + "&body=" +
                          HttpUtility.UrlEncode("Version: " + PspGlobalConfiguration.CurrentVersion + "\nTitle: " +
                                                ElfConfig.GameTitle + "\n") + "&labels[]=" +
                          HttpUtility.UrlEncode("Games"));
        }
        ...
        private void LanguageUpdated()
        {
            ...
            foreach (var LanguagePair in LanguagePairs)
            {
                ...
                LanguagePair.Key.Checked =
                    (LanguagePair.Value.CompareInfo == Thread.CurrentThread.CurrentUICulture.CompareInfo);
            }
        ...
        {
            PauseResume(() => { InjectContext.NewInstance<FunctionViewerForm>().ShowDialog(); });
        }

        private void DebugTextureViewer_Click(object sender, EventArgs e)
        {
            PauseResume(() => { InjectContext.NewInstance<TextureViewerForm>().ShowDialog(); });
        }
        ...
        {
            PauseResume(() => { InjectContext.NewInstance<CheatsForm>().ShowDialog(); });
        }
