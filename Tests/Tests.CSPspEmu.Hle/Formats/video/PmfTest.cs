		...
		public void LoadTest()
		{
			...
			fixed (byte* At3DataPtr = At3Data)
			{
				...
				for (int n = 0; n < 1000; n++, frame++)
				{
					...
					int channels = 0;
					short[] SamplesData;
					File.WriteAllBytes(@"c:\isos\psp\out\samples" + frame + ".in", new MemoryStream().WriteBytes(PointerUtils.PointerToByteArray(At3Ptr, FrameSize)).ToArray());
					...
					File.WriteAllBytes(@"c:\isos\psp\out\samples" + frame + ".out", new MemoryStream().WriteStructVector(SamplesData).ToArray());
					At3Ptr += FrameSize;
				}
			}
