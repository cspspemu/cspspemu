				...
				if (MpegPsDemuxer.HasMorePackets)
				{
					...
					switch (Packet.Type)
					{
						case Formats.video.MpegPsDemuxer.ChunkType.ST_Video1:
							...
							if (DumpStreams) FileUtils.CreateAndAppendStream(@"c:\isos\psp\out\video.stream", Info.Stream.Slice());
							Info.Stream.Slice().CopyToFast(VideoStream);
							break;
						case Formats.video.MpegPsDemuxer.ChunkType.ST_Private1:
							if (DumpStreams) FileUtils.CreateAndAppendStream(@"c:\isos\psp\out\audio.stream", Info.Stream.Slice());
