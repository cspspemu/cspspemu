...
namespace CSPspEmu.Core.Crypto
{
    ...
    {
#if false //#define GETuint(pt) (((uint)(pt)[0] << 24) ^ ((uint)(pt)[1] << 16) ^ ((uint)(pt)[2] <<  8) ^ ((uint)(pt)[3]))
//#define PUTuint(ct, st) { (ct)[0] = (u8)((st) >> 24); (ct)[1] = (u8)((st) >> 16); (ct)[2] = (u8)((st) >>  8); (ct)[3] = (u8)(st); }
        ...
        public static void AES_cbc_encrypt(AES_ctx* ctx, byte* src, byte* dst, int size)
        {
            ...
            fixed (byte* pblockBuff = blockBuff)
            {
                int i;
                for (i = 0; i < size; i += 16)
                {
        ...
        public static void AES_cbc_decrypt(AES_ctx* ctx, byte* src, byte* dst, int size)
        {
            ...
            fixed (byte* pblockBuff = blockBuff)
            fixed (byte* pBlockBuffPrevious = blockBuffPrevious)
            {
                Memcpy(pblockBuff, src, 16);
                Memcpy(pBlockBuffPrevious, src, 16);
                ...
                for (i = 16; i < size; i += 16)
                {
                    ...
                    xor_128(dst, pBlockBuffPrevious, dst);
                    //step5: swap buffers
                    Memcpy(pBlockBuffPrevious, pblockBuff, 16);
