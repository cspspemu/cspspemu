            ...
            if (playback_handle == IntPtr.Zero)
            {
                try
                {
                    ...
                    fixed (IntPtr* hw_params_ptr = &hw_params)
                    {
                        ...
                        Assert("snd_pcm_open",
                            Alsa.snd_pcm_open(playback_handle_ptr, Device,
                        ...
                        Assert("snd_pcm_hw_params_set_access",
                            Alsa.snd_pcm_hw_params_set_access(playback_handle, hw_params,
                                Alsa.snd_pcm_access.SND_PCM_ACCESS_RW_INTERLEAVED));
                        Assert("snd_pcm_hw_params_set_format",
                            Alsa.snd_pcm_hw_params_set_format(playback_handle, hw_params,
                                Alsa.snd_pcm_format.SND_PCM_FORMAT_S16_LE));
                        Assert("snd_pcm_hw_params_set_rate_near",
                            Alsa.snd_pcm_hw_params_set_rate_near(playback_handle, hw_params, &rate, null));
                        ...
                        Assert("snd_pcm_hw_params_set_channels",
                            Alsa.snd_pcm_hw_params_set_channels(playback_handle, hw_params, channels));

                        Assert("snd_pcm_hw_params_set_periods",
                            Alsa.snd_pcm_hw_params_set_periods(playback_handle, hw_params, periods, 0));
                        Assert("snd_pcm_hw_params_set_period_size",
                            Alsa.snd_pcm_hw_params_set_period_size(playback_handle, hw_params, periodsize, null));
                        Assert("snd_pcm_hw_params_set_buffer_size",
                            Alsa.snd_pcm_hw_params_set_buffer_size(playback_handle, hw_params, 4 * periodsize));
            ...
            get
            {
                try
                {
                    if (Platform.IsPosix)
                    {
                        ...
                        var Result = Alsa.snd_pcm_open(&temp_playback_handle, Device,
                            Alsa.snd_pcm_stream_t.SND_PCM_LB_OPEN_PLAYBACK, 0);
                        if (Result >= 0)
                        {
