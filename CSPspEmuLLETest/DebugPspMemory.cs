...
namespace CSPspEmuLLETest
{
	public class DebugPspMemory : LazyPspMemory
	{
		...
		{
			if (!PspMemory.IsAddressValid(Address)) return (byte)Dma.ReadDMA(1, Address);
		...
		{
			if (!PspMemory.IsAddressValid(Address)) return (ushort)Dma.ReadDMA(2, Address);
		...
		{
			if (!PspMemory.IsAddressValid(Address)) return (uint)Dma.ReadDMA(4, Address);
		...
		public override void Write1(uint Address, byte Value)
		{
			...
			if (!PspMemory.IsAddressValid(Address)) { Dma.WriteDMA(1, Address, Value); return; }
		...
		{
			TrackWrite(Address, Value);
			if (!PspMemory.IsAddressValid(Address)) { Dma.WriteDMA(2, Address, Value); return; }
		...
		{
			TrackWrite(Address, Value);
			if (!PspMemory.IsAddressValid(Address)) { Dma.WriteDMA(4, Address, Value); return; }
